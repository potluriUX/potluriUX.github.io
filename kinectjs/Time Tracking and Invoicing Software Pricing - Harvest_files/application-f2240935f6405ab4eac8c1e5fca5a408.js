...
// @TODO: this should definitely go into application.js once the async loader is history!
$( '[rel="fancybox-youtube"]' ).each( function()
{
  link = $( this );

  // retrieve the video id from the url

  slug = link.attr( 'href' ).split( /\?v=([\w_-]*)/ )[ 1 ];

  link.fancybox(
  {
    content: '<iframe width="853" height="480" src="http://www.youtube.com/embed/' + slug + '?hl=en&fs=1&rel=0&autoplay=1" frameborder="0" style="display:block" allowfullscreen></iframe>',
...
$( '[rel="fancybox-wistia"]' ).each( function()
{
  link = $( this );

  // retrieve the video id from the url
  slug = link.attr( 'href' ).split( /iframe\/([\w_-]*)/ )[ 1 ];

  link.fancybox(
  {
    content: '<iframe width="853" height="480" src="http://fast.wistia.net/embed/iframe/' + slug + '?popover=true" frameborder="0" style="display:block" allowfullscreen></iframe>',
    hideOnContentClick: false,
    scrolling: false,
    autoDimensions: true,
    overlayColor: '#000',
...
$( document ).ready(function(){
  ...
  if( $('#recent_tweets').length ){
    $("#recent_tweets").after('<div id="recent_tweets_loading" class="tweets-loading"><img src="/assets/loading.gif" /> <span>Loading Tweets</span></div>');
  ...
  //
  // Do these things on the homepage
  //
  if( $("#home-intro").length ){
    ...
    video.attr("src", "").height($("#home-intro").height());
    ...
    // Hide the video
    $(".button-close-video").on("click", function(e) {
      e.preventDefault();
      html.removeClass("video-is-visible");
      video.attr("src", "");
    });

    // Autoplay the video on the homepage
    if (html.hasClass("autoplay-video")) {
  ...
  //
  // Do these things on the Forecast homepage
  //
  $('#visualize-projects-toggle').on('click', function() {
    $('#visualize-projects-toggle').addClass('active');
    $('#visualize-team-toggle').removeClass('active');
    $('#visualize-projects').show();
    $('#visualize-team').hide();
  });
  $('#visualize-team-toggle').on('click', function() {
    $('#visualize-team-toggle').addClass('active');
    $('#visualize-projects-toggle').removeClass('active');
    $('#visualize-team').show();
    $('#visualize-projects').hide();
  });
  $('#visualize-team').on('click', function() {
    $('#visualize-projects-toggle').addClass('active');
    $('#visualize-team-toggle').removeClass('active');
    $('#visualize-projects').show();
    $('#visualize-team').hide();
  });
  $('#visualize-projects').on('click', function() {
    $('#visualize-team-toggle').addClass('active');
    $('#visualize-projects-toggle').removeClass('active');
    $('#visualize-team').show();
    $('#visualize-projects').hide();
  ...
  $('#assignment-demo').appear();
  $(document.body).on('appear', '#assignment-demo', function() {
    $(this).addClass('animate');
  });
  $(document.body).on('disappear', '#assignment-demo', function() {
    $(this).removeClass('animate');
  });
  $('#assignment-demo').on('mousedown', function() {
    $(this).removeClass('animate');
  });
  $('#assignment-demo').on('mouseup', function() {
    $(this).addClass('animate');
  });

  (function() {
    var modal = $('#forecast-sign-up-modal'),
        form  = $('#forecast-sign-up-form');
    $('.forecast-sign-up-trigger').on('click', function() {
      modal.fadeIn('fast');
    });
    $('.forecast-sign-up-close').on('click', function() {
      modal.fadeOut('fast');
    });
    $(document.body).live('keyup', function(e) {
      if(e.keyCode === 27) {
        modal.fadeOut('fast');
      }
    });
  ...
  window.sign_in = {
    ...
    submit_form : function(form){
      ...
      $("#help_document_submit").hide().after("<div id=\"forgot_loader\" style=\"line-height: 30px; height: 30px;\"><img src=\"/assets/loading.gif\" style=\"vertical-align:middle;\" /></div>");
  ...
  if ($('.instant-search').length) {
    ...
    $('.instant-search').each(function(){
        ...
        if (result_el) {
          ...
          var search = function(evt){
            ...
            var search_text = $("<div />").text(search_el.val()).html();

            if(evt.which == 40 && result_el.is(':visible') && result_el.find("ul") ){
              search_el.blur();
              search_results.highlight();
            }
            else if ( $.trim( search_text ).length > 2 && search_text != last_term){
                    if ( !window.instant_searching ) {
                      ...
                      window.instant_searching = true;
                        ...
                        if(easter_reg.test(last_term)){
                          ...
                          if(last_term[0]==="g"){easter_src = "/assets/support/giraffe.jpg";}
                          else if(last_term[0]==="a" || last_term[0]==="o"){easter_src = "/assets/support/orangutang.jpg";}
                          else {easter_src = "/assets/support/doug.gif";}
                          ...
                          jQuery.getJSON(
                            ...
                            function(data){
                              ...
                              if (data && data.length > 0){
                                ...
                                result_el.html('<span class="error">Sorry, no results found for <b>&ldquo;' + $.trim( search_text ) + '.&rdquo;</b></span>');
                              }
    ...
    if ($('.instant-search').val().length > 1) {
      $('.instant-search').trigger('keyup')
    }
  }
